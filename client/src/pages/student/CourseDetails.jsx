...
// Helper to format lecture duration (assumes duration is in minutes)
...
const CourseDetails = () => {
  ...
  const { allCourses ,calculateRating, calculateNoOfLectures,calculateChapterTime, 
 ...
 //this function will allow us to buy the course 
  const enrollCourse = async ()=>{
           try {
              ...
              if(data.success)
              {
                 ...
                 window.location.replace(session_url) //go to that url
              }
              else{
                toast.error(data.message)
              }
  ...
  return courseData ? (
    ...
    relative items-start justify-between md:px-36 px-8 md:pt-30
    pt-20 text-left"
      >
  <div className="absolute top-0 left-0 w-full h-[260px] z-[-1] bg-gradient-to-b from-cyan-100/70" />
        ...
        text-course-details-heading-small font-semibold text-gray-800"
          ...
          <div className="flex items-center space-x-2">
           ...
           <p className='text-sm'>Course by <span className='text-blue-600 underline'>{courseData?.instructor?.name ?? 'Unknown instructor'}</span></p>
            ...
            <div className='pt-5'>
              {courseData.courseContent.map((chapter,index)=>(
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded overflow-hidden'>
                  ...
                  >
                    <ul className="list-disc md:pl-10 pl-4 pr-4
                    py-2 text-gray-600
        ...
        md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          ...
          <div className="p-5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-red-500"><span className='font-mediums'>5 days</span> left at this price!</p>
            </div>
